...
object SettingsManager {
    ...
    private const val KEY_BRAKE_AUTOHOLD_VOICE = "key_brake_autohold_voice"
    ...
    // 5-2 & 5-3 BSD / LDP 경고음 모드: "BEEP", "VOICE_RECOMMENDED", "VOICE_CUSTOM"
    ...
    // 8번 부팅 시 자동 실행 설정 키
    private const val KEY_BOOT_MEDIA_PKG = "key_boot_media_pkg"
    private const val KEY_BOOT_MEDIA_DELAY = "key_boot_media_delay"
    private const val KEY_BOOT_CUSTOM_PKG = "key_boot_custom_pkg"
    private const val KEY_BOOT_CUSTOM_DELAY = "key_boot_custom_delay"
    ...
    fun getBsdCustomText(context: Context): String = getPrefs(context).getString(KEY_BSD_CUSTOM_TEXT, "후측방 충돌 위험이 감지되었습니다.") ?: "후측방 충돌 위험이 감지되었습니다."
    fun setBsdCustomText(context: Context, text: String) = getPrefs(context).edit().putString(KEY_BSD_CUSTOM_TEXT, text).apply()

    fun getLdpCustomText(context: Context): String = getPrefs(context).getString(KEY_LDP_CUSTOM_TEXT, "차선을 이탈하여 조향을 보조합니다.") ?: "차선을 이탈하여 조향을 보조합니다."
    fun setLdpCustomText(context: Context, text: String) = getPrefs(context).edit().putString(KEY_LDP_CUSTOM_TEXT, text).apply()

    // 5-4 10대 차량 상태 음성 멘트 Get/Set
    ...
    fun getBootMediaPkg(context: Context): String = getPrefs(context).getString(KEY_BOOT_MEDIA_PKG, "com.android.music") ?: "com.android.music"
    fun setBootMediaPkg(context: Context, pkg: String) = getPrefs(context).edit().putString(KEY_BOOT_MEDIA_PKG, pkg).apply()

    fun getBootMediaDelay(context: Context): Int = getPrefs(context).getInt(KEY_BOOT_MEDIA_DELAY, 5)
    fun setBootMediaDelay(context: Context, sec: Int) = getPrefs(context).edit().putInt(KEY_BOOT_MEDIA_DELAY, sec).apply()

    fun getBootCustomPkg(context: Context): String = getPrefs(context).getString(KEY_BOOT_CUSTOM_PKG, "com.skt.tmap.ku") ?: "com.skt.tmap.ku"
    fun setBootCustomPkg(context: Context, pkg: String) = getPrefs(context).edit().putString(KEY_BOOT_CUSTOM_PKG, pkg).apply()

    fun getBootCustomDelay(context: Context): Int = getPrefs(context).getInt(KEY_BOOT_CUSTOM_DELAY, 8)
    fun setBootCustomDelay(context: Context, sec: Int) = getPrefs(context).edit().putInt(KEY_BOOT_CUSTOM_DELAY, sec).apply()
