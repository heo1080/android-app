...
import com.byd.dolphin.autoassistant.activity.InsideLightToggleActivity
...
object InsideLightManager {
    ...
    /**
     * 홈 화면에 실내등 스위치 바로가기 아이콘 생성
     */
    fun createHomeScreenShortcut(context: Context) {
        if (!ShortcutManagerCompat.isRequestPinShortcutSupported(context)) {
            Toast.makeText(context, "현재 런처는 바로가기 생성을 지원하지 않습니다.", Toast.LENGTH_SHORT).show()
            return
        }

        val launchIntent = Intent(context, InsideLightToggleActivity::class.java).apply {
            action = Intent.ACTION_MAIN
            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP)
        }

        val shortcutInfo = ShortcutInfoCompat.Builder(context, "shortcut_inside_light_toggle")
            .setShortLabel("실내등 스위치")
            .setLongLabel("전체 실내등 켜기/끄기 토글")
            .setIcon(IconCompat.createWithResource(context, R.drawable.ic_light_toggle))
            .setIntent(launchIntent)
            .build()

        ShortcutManagerCompat.requestPinShortcut(context, shortcutInfo, null)
        Toast.makeText(context, "홈 화면에 '실내등 스위치' 바로가기를 추가했습니다.", Toast.LENGTH_SHORT).show()
    }
}
