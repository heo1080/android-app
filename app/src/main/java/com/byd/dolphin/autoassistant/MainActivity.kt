...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 2. 0번 요구사항: ADB 프로토콜(A_CNXN + A_AUTH) 전송 -> 'USB 디버깅을 허용하시겠습니까?' 팝업 트리거
        performAutoAdbGrant()
    }
    ...
    // =========================================================================
    // 0번 요구사항: NativeAdbClient 실행 -> USB 디버깅 팝업 트리거 및 권한 획득
    // =========================================================================
    private fun performAutoAdbGrant() {
        ...
        AdbPermissionManager.autoGrantPermissionsOnLaunch(this) { success, msg ->
            if (success) {
                tvStatus.text = "🟢 실시간 감시 중 (모든 차량 권한 승인 완료)"
                tvStatus.setTextColor(Color.parseColor("#00E676"))
                startDolphinService()
            } else {
                tvStatus.text = "⚠️ ADB 권한 확인 필요 (5555 포트 연결 또는 버그제거 1회 연결)"
    ...
    private fun setupDpiAdbSubScreen() {
        ...
        findViewById<Button>(R.id.btnAutoGrantAdb).setOnClickListener {
            Toast.makeText(this, "로컬 ADB(5555) 연결 시도 중... 화면 팝업을 확인하세요", Toast.LENGTH_SHORT).show()
            performAutoAdbGrant()
        }
    }
    ...
    private fun setupHudSubScreen() {
        ...
        findViewById<Button>(R.id.btnSendAppToCluster).setOnClickListener {
            showAppPicker("계기판으로 보낼 앱 선택") { pkg, _ ->
